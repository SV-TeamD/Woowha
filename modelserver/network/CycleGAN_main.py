...
def get_args():
    ...
    parser.add_argument(
        "--animation_image_dir", default=Config.animation_image_dir, help="Path to animation images"
    )
    ...
    parser.add_argument(
        "--test_image_path", default=Config.test_photo_image_dir, help="Path to test photo images"
    )
    ...
    parser.add_argument(
        "--num_epochs", type=int, default=Config.num_epochs, help="Number of training epochs"
    )

    parser.add_argument("--batch_size", type=int, default=Config.batch_size)

    parser.add_argument(
        "--use_edge_smoothed_images",
        action="store_true",
        help="Use this argument to use edge smoothed images in training",
    )

    parser.add_argument(
        "--test_animation_to_photo",
        action="store_true",
...
def main():
    ...
    if args.test:
        ...
        test_images = load_image_dataloader(
            root_dir=args.test_image_path, batch_size=Config.batch_size * 2, shuffle=False
        )
        ...
        tvutils.save_image(
            image_batch, "test_images.jpg", nrow=4, padding=2, normalize=True, range=(-1, 1)
        )
        tvutils.save_image(
            new_images, "generated_images.jpg", nrow=4, padding=2, normalize=True, range=(-1, 1)
        )
        ...
        if args.load_data_on_ram:
            photo_images = load_image_dataloader_on_RAM(
                root_dir=args.photo_image_dir, batch_size=args.batch_size
            )
            animation_images = load_image_dataloader_on_RAM(
                root_dir=args.animation_image_dir, batch_size=args.batch_size
            )
            edge_smoothed_images = load_image_dataloader_on_RAM(
        ...
        trainer = CycleGANTrainer(
            ...
            use_edge_smoothed=args.use_edge_smoothed_images,
