...
class CycleGANTrainer:
    def __init__(
        ...
        D_y,
        photo_image_loader,
        animation_image_loader,
        edge_smoothed_image_loader,
        lambda_cycle=Config.lambda_cycle,
        lambda_identity=Config.lambda_identity,
        use_edge_smoothed=False,
        ...
        self.G_optimizer = optim.Adam(
            self.G.parameters(), lr=Config.lr, betas=(Config.adam_beta1, 0.999)
        )
        self.F_optimizer = optim.Adam(
            self.F.parameters(), lr=Config.lr, betas=(Config.adam_beta1, 0.999)
        )
        self.D_x_optimizer = optim.Adam(
            self.D_x.parameters(), lr=Config.lr, betas=(Config.adam_beta1, 0.999)
        )
        self.D_y_optimizer = optim.Adam(
            self.D_y.parameters(), lr=Config.lr, betas=(Config.adam_beta1, 0.999)
        )
    ...
    def train(
        ...
        save_path="checkpoints/CycleGAN/",
    ):
        ...
        for init_epoch in range(self.curr_initialization_epoch, initialization_epochs):
            ...
            for ix, ((photo_images, _), (animation_images, _)) in enumerate(
                ...
                if (ix + 1) % self.print_every == 0:
                    print(
                        "Initialization Phase Epoch {0} Iteration {1}: Content Loss: {2:.4f}".format(
                            init_epoch + 1, ix + 1, epoch_loss / (ix + 1)
                        )
                    )

            print(
                "Initialization Phase [{0}/{1}], {2:.4f} seconds".format(
                    init_epoch + 1, initialization_epochs, time.time() - start
                )
            )
        ...
        for epoch in range(self.curr_epoch, num_epochs):
            ...
            for ix, (
                (photo_images, _),
                (animation_images, _),
                (edge_smoothed_images, _),
            ) in enumerate(
                zip(
                    self.photo_image_loader,
                    self.animation_image_loader,
                ...
                (
                    ...
                    loss_cycle,
                    loss_identity,
                ) = self.train_step(photo_images, animation_images, edge_smoothed_images)
                ...
                if (ix + 1) % self.print_every == 0:
                    print(
                        "Training Phase Epoch {0} Iteration {1}: loss_D_x: {2:.4f} loss_D_y: {3:.4f} loss_G: {4:.4f} loss_F: {5:.4f} "
                        "loss_cycle: {6:.4f} loss_identity: {7:.4f}".format(
                            epoch + 1,
                            ix + 1,
                            epoch_loss_D_x / (ix + 1),
                            epoch_loss_D_y / (ix + 1),
                            epoch_loss_G_GAN / (ix + 1),
                            epoch_loss_F_GAN / (ix + 1),
                            epoch_loss_cycle / (ix + 1),
                            epoch_loss_identity / (ix + 1),
                        )
                    )  # print progress

            self.curr_epoch += 1
            print(
                "Training Phase [{0}/{1}], {2:.4f} seconds".format(
                    self.curr_epoch, num_epochs, time.time() - start
                )
            )

        # Training finished, save checkpoint
        self.save_checkpoint(
            os.path.join(save_path, "checkpoint-epoch-{0}.ckpt".format(num_epochs))
        )
