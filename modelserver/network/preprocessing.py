...
def perform_edge_smoothing(
    img_path, save_path, kernel_size=5, canny_threshold1=100, canny_threshold2=200
):
    """
    ...
    for i in range(len(idx[0])):
        ...
        smoothed_img[idx[0][i], idx[1][i], 0] = np.sum(
            np.multiply(
                pad_img[
                    idx[0][i] : idx[0][i] + kernel_size, idx[1][i] : idx[1][i] + kernel_size, 0
                ],
                gauss,
            )
        )
        smoothed_img[idx[0][i], idx[1][i], 1] = np.sum(
            np.multiply(
                pad_img[
                    idx[0][i] : idx[0][i] + kernel_size, idx[1][i] : idx[1][i] + kernel_size, 1
                ],
                gauss,
            )
        )
        smoothed_img[idx[0][i], idx[1][i], 2] = np.sum(
            np.multiply(
                pad_img[
                    idx[0][i] : idx[0][i] + kernel_size, idx[1][i] : idx[1][i] + kernel_size, 2
                ],
                gauss,
            )
        )
...
def main():
    ...
    if args.image_type_to_process == "photo":
        ...
        elif not os.path.isdir(args.photo_image_source_path) or not os.path.isdir(
            args.photo_image_save_path
        ):
            parser.error(
                "--photo_image_source_path and --photo_image_save_path must be existing directories."
            )
        else:
            preprocess_photo_images(
                args.photo_image_source_path,
                args.photo_image_save_path,
        ...
        if (
            args.animation_image_source_path is None
            or args.animation_image_save_path is None
            or args.animation_edge_smoothed_save_path is None
        ):
            parser.error(
                "--animation_image_source_path, --animation_image_save_path and --animation_edge_smoothed_save_path is required."
            )
        elif (
            not os.path.isdir(args.animation_image_source_path)
            or not os.path.isdir(args.animation_image_save_path)
            or not os.path.isdir(args.animation_edge_smoothed_save_path)
        ):
            parser.error(
                "--animation_image_source_path, --animation_image_save_path and --animation_edge_smoothed_save_path must be existing directories."
            )
        else:
            preprocess_animation_images(
                args.animation_image_source_path,
                args.animation_image_save_path,
                args.animation_edge_smoothed_save_path,
                target_size=args.target_size,
            )
