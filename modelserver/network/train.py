...
class Trainer:
    def __init__(
        ...
        # in case generator and discriminator are not using Config.device
        ...
        self.gen_optimizer = optim.Adam(
            self.generator.parameters(), lr=Config.lr, betas=(Config.adam_beta1, 0.999)
        )
        ...
        if not lsgan:
            self.disc_criterion = nn.BCEWithLogitsLoss().to(
                Config.device
            )  # for discriminator GAN loss
            self.gen_criterion_gan = nn.BCEWithLogitsLoss().to(
                Config.device
            )  # for generator GAN loss
        else:
    ...
    def train(
        ...
        for epoch in range(self.curr_epoch, num_epochs):
            ...
            for ix, (
                ...
                loss_D, loss_G, loss_content = self.train_step(
                    animation_images, edge_smoothed_images, photo_images
                )
                epoch_loss_D += loss_D
                epoch_loss_G += loss_G
                ...
                if (ix + 1) % self.print_every == 0:
                    print(
                        "Training Phase Epoch {0} Iteration {1}, loss_D: {2:.4f}, "
                        "loss_G: {3:.4f}, loss_content: {4:.4f}".format(
                            epoch + 1,
                            ix + 1,
                            epoch_loss_D / (ix + 1),
                            epoch_loss_G / (ix + 1),
    ...
    def train_step(self, animation_images, edge_smoothed_images, photo_images):
        ...
        loss_content = self.content_loss_weight * self.gen_criterion_content(
    ...
    def initialize_step(self, photo_images):
        self.generator.zero_grad()
        x_features = self.feature_extractor(
            (photo_images + 1) / 2
        ).detach()  # move [-1, 1] to [0, 1]
        Gx = self.generator(photo_images)
        Gx_features = self.feature_extractor((Gx + 1) / 2)  # move [-1, 1] to [0, 1]

        content_loss = self.content_loss_weight * self.gen_criterion_content(
            Gx_features, x_features
        )
